...
{    
    ...
    NSString* appID = @"5c954267b9d8170018db87b7";
...
-( void )showV4VCV:( BOOL )isIncentivised
{
   ...
   // NSDictionary *options = @{VunglePlayAdOptionKeyIncentivized: @(isIncentivised)};
    //NSDictionary *options = @{VunglePlayAdOptionKeyOrientations: @(isIncentivised)};

    NSString* appID = @"5c9eb178b9d8170018dc3676";
    NSString* placementID = @"FULLVIDEO-3775752";
    NSError *error;
    printf("trying to play ad..");
    [sdk startWithAppId:appID error:&error];
    ...
    NSDictionary *options = @{VunglePlayAdOptionKeyOrientations: @(UIInterfaceOrientationMaskLandscape)};
     printf("trying to play ad..1");
    [sdk playAd:localViewController options:options placementID: @"FULLVIDEO-3775752" error:&error];
     printf("trying to play ad..2");
    if (error) {
        NSLog(@"Error encountered playing ad: %@", error);
       
    }
    
   // [sdk playAd:localViewController options:nil placementID:@"BANNERAD-9943903" error:&error];
    
    printf("this statement worked");
    if ( error )
    {
        NSLog( @"Error encountered playing ad: %@", error );
    }
